...
class ParticleDiagnosticICS(OpenPMDDiagnostic) :
    """
    Class that defines the particle diagnostics for ICS simulations.
    The diagnostic stores the trajectories and momenta of particles in
    memory until asked to be dumped by the user.
    """

    def __init__(self, period=None, species={}, comm=None,
        particle_data=("position", "momentum", "weighting", "gamma"),
    ...
    def write( self, iteration ):
        """
        Store particle data in a buffer on every iteration.
        Additionally, check if the data should be written at this
        iteration (based on iteration) and if yes, write it.
        ...
        """
        # First store data in a buffer
        self.store_iteration(iteration)
        ...
        if iteration % self.period == 0 \
            ...
            # Write all the buffered data into the hdf5 file
            # with the current iteration name
            filename = "data%08d.h5" % iteration
            fullpath = os.path.join(self.write_dir, "hdf5", filename)
            for buffer_iteration in self.data_buffer.keys():
                self.write_hdf5(buffer_iteration, fullpath)

            # And clear the buffer again
            self.data_buffer = {}

    def store_iteration(self, iteration):
        """
        ...
        for species_name in self.species_names_list:
            ...
            # Store the datasets for each particle datatype
            self.store_particles(species, iteration, n_rank, Ntot, select_array,
                                 self.array_quantities_dict[species_name])
    ...
    def write_hdf5( self, iter_to_write, filename ) :
        """
        ...
        ---------
        iter_to_write : int
             The current iteration number to be written
        ...
        if self.rank == 0:
            ...
            self.setup_openpmd_file( f, iter_to_write, iter_to_write*self.dt, self.dt)
        ...
        for species_name in self.species_names_list:
            ...
            self.write_particles( species_grp, species, iter_to_write,
                                  self.array_quantities_dict[species_name] )
    ...
    def write_particles( self, species_grp, species, iter_to_write, particle_data ) :
        """
        ...
        iter_to_write: int
            Iteration number to be currently written to disk
    ...
    def store_particles( self, species, iteration, n_rank,
                         Ntot, select_array, particle_data ) :
        """
        Store all the particle data sets for one given species
        ...
        """
        # Create buffer entry
        self.data_buffer[iteration][species] = {}

        # Loop through the quantities and write them
        for quantity in particle_data:
            try:
                self.store_dataset(iteration, species,
                                   quantity, n_rank, Ntot, select_array )
            except ValueError:
                raise ValueError("Invalid string in %s of species"
                                    %(quantity))

    def store_dataset( self, iteration, species, quantity,
        ...
        quantity_array = self.get_dataset( species, quantity, select_array,
                                           n_rank, Ntot )
        # Keep the data on rank 0 only
        if self.rank == 0:
            self.data_buffer[iteration][species][quantity] = quantity_array
    ...
    def write_dataset( self, species_grp, species, path, quantity,
                       iter_to_write ) :
        """
        Write a given dataset. Only done by rank 0!
        ...
        iter_to_write: int
            Iteration number to be currently written
        """
        # Create the dataset and setup its attributes
        if self.rank == 0:
            # Recall the dataset
            quantity_array = self.data_buffer[iter_to_write][species][quantity]
            Ntot = len(quantity_array)
